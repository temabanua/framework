...
This typically indicates that your device does not have a healthy Internet connection at the moment. The client will operate in offline mode until it is able to successfully connect to the backend.`;
  ...
  // main.js
  var firebaseConfig = {
    apiKey: "AIzaSyCkUbJrbIoIHZo7WQNjmx6SeIDbYQg5VR0",
    authDomain: "login.temabanua.com",
    projectId: "blogger-login-e21b0",
    storageBucket: "blogger-login-e21b0.firebasestorage.app",
    messagingSenderId: "948689976249",
    appId: "1:948689976249:web:faad361784bfb987f75468",
    measurementId: "G-PQN97Q7MB4"
  };
  ...
  $(document).ready(function() {
    ...
    const listEl = $("#download-item-file");
    const notLoggedInEl = $("#not-logged-in");
    onAuthStateChanged(auth, async (user) => {
      if (user) {
        loadingEl.css("display", "flex");
        notLoggedInEl.css("display", "none");
        listEl.css("display", "none");
        try {
          const q2 = query(collection(db, "downloads"), orderBy("createdAt", "desc"));
          const querySnapshot = await getDocs(q2);
          loadingEl.css("display", "none");
          listEl.css("display", "flex");
          if (querySnapshot.empty) {
            listEl.html("<div class='no-dn'>There are no download items for this Account.</div>");
            return;
          }
          listEl.empty();
          querySnapshot.forEach((doc2) => {
            ...
            const itemHtml = `
            <a href="${data.fileUrl}" target="_blank" title="${data.fileName}" class="index-post">
              <div class='post-list'>
                <div class='item-free-temabanua'>
                  <div class="post-image-relative">
                    <div class="binuang_post-image">
                      <img class="post-thumb" src="${data.imageUrl}" alt="${data.fileName}">
                    </div>
                  </div>
                  <div class="product-info gap16">
                    <div class='item-free-dn'>
                      <h2 class='post-title'>${data.fileName}</h2>
          ...
          loadingEl.text("Failed to load download item.");
          console.error("Error getting downloads: ", error);
        }
      } else {
        loadingEl.css("display", "none");
        listEl.css("display", "none");
        notLoggedInEl.css("display", "block");
      }
    });
  });
  ...
  var productId = itemDataId + "";
  var selectedRating = 0;
  var currentFilter = "latest";
  function renderStarInput(rating = 0) {
    const container = $("#star-input").empty();
    let hoverRating = 0;
    for (let i = 1; i <= 5; i++) {
      const star = $(`<span><svg width="24" height="24" viewBox="0 0 24 24" class="star-icon" data-value="${i}">
        <path d="M12 2L15 8.6 22 9.3 17 14.2 18.2 21 12 17.8 5.8 21 7 14.2 2 9.3 9 8.6 12 2Z"
        fill="${i <= rating ? "#fbbf24" : "none"}" stroke="#fbbf24" stroke-width="2" />
      </svg></span>`);
      star.on("mouseenter", () => {
        hoverRating = i;
        updateStarColors(hoverRating);
      });
      star.on("mouseleave", () => updateStarColors(selectedRating));
      star.on("click", () => {
        selectedRating = i;
        $("#user-rating").val(i);
        updateStarColors(i);
    ...
    function updateStarColors(activeRating) {
      container.find("svg").each(function(index) {
        $(this).find("path").attr("fill", index + 1 <= activeRating ? "#fbbf24" : "none");
      });
    }
  }
  function renderStars(count) {
    return [...Array(5)].map((_, i) => `
    <svg width="16" height="16" viewBox="0 0 24 24" fill="${i < count ? "#fbbf24" : "none"}" stroke="#fbbf24" stroke-width="2">
      <path d="M12 2L15 8.6 22 9.3 17 14.2 18.2 21 12 17.8 5.8 21 7 14.2 2 9.3 9 8.6 12 2Z"/>
    </svg>`).join("");
  }
  ...
  function filterAllowedLinks(text) {
    const allowedDomains = ["www.temabanua.com", "help.temabanua.com"];
  ...
  async function loadRating(filter = "latest") {
    let q2 = query(collection(db, "ratings"), where("productId", "==", productId));
    if (filter === "latest") q2 = query(q2, orderBy("timestamp", "desc"));
    else if (filter === "highest") q2 = query(q2, orderBy("rating", "desc"));
    else if (filter === "lowest") q2 = query(q2, orderBy("rating", "asc"));
    $("#loading").css("display", "flex");
    $("#rating-display").css("display", "none");
    try {
        ...
        <div class='item-reviews'>
          <div class='head-reviews'>
            <div class='item-title-r'>
              <div class='item-icon-r'>${renderStars(data.rating)}</div>
              <p class="item-text-r">for <strong>${data.reviewLabel || "Feedback"}</strong></p>
            </div>
            <div class='item-date-r'>
              <p class='item-name-r'>By <a href="/p/user.html?uid=${data.userId}">${data.userName || "Users"}</a>
              <span class='item-date-grey'>${data.timestamp?.toDate ? timeAgo(data.timestamp.toDate()) : "Just now"}</span></p>
            </div>
          </div>
          <div class='body-reviews'>
            <p>${filterAllowedLinks(data.comment || "")}</p>
          </div>
        </div>`;
      });
      const avg = count ? (total / count).toFixed(1) : "0.0";
      $("#rating-avg").html(`<strong>${avg}</strong> <div class="flex">${renderStars(Math.round(avg))}</div>`);
      $("#rating-count").text(`${count}`);
      $("#rating-comments").html(html);
    } catch (error) {
      console.error("Failed to load ratings:", error);
    } finally {
      $("#loading").hide();
      $("#rating-display").css("display", "flex");
    }
  }
  async function checkIfUserRated() {
    const user = auth.currentUser;
    if (!user) {
      $("#rating-form, #already-reviewed").hide();
      $("#login-btn-d").show();
      return;
    }
    const docId = `${productId}_${user.uid}`;
    try {
      const docSnap = await getDoc(doc(db, "ratings", docId));
      if (docSnap.exists()) {
        $("#rating-form").hide();
        $("#already-reviewed").css("display", "flex");
      } else {
  ...
  function showToast(message, type = "success") {
    ...
    $("body").append(toast);
    setTimeout(() => toast.fadeOut(300, () => toast.remove()), 3e3);
  }
})();
